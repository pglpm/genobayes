...
savedir <- 'allmc1e6_1snp_1snp_unif/' # directory for saving results
filename <- 'freqsnp-1_1_unif-' # filename prefix

cores <- 30 # for parallel processing
mciterations <- 1e6 # number of Monte-Carlo samples

symptoms <- as.list(4:dim(data)[2]) # list of gene indices in data
namesymptoms <- sapply(symptoms,function(x){paste0(x[1]-3)})
prefixsymptoms <- 'snp_' # for filename
