...
symptoms <- as.list(studysnp+3) # first snp
namesymptoms <- sapply(symptoms,function(x){paste0(x[1]-3)})
prefixsymptoms <- 'snpa_' # for filename

symptomvariants <- list(0,1)
namesymptomvariants <- c('A1','B1')

snps <- symptoms # second snp
namesnps <- namesymptoms
## namesnps <- colnames(data)[(1:94)+3] # this was writing full SNP names
prefixsnps <- 'snpb_' # for filename

snpvariants <- symptomvariants # list of allele values
namesnpvariants <- c('A2','B2') # allele names
...
#erf <- function(x) 2 * pnorm(x * sqrt(2)) - 1
    ...
    maxspread <- max(abs(c(spreads)))
        ...
        rownames(diffdata) <- c('EVabs_B','EV_B','SD_B')
        colnames(diffdata) <- namesnpcombos
        margdata <- rbind(evs,sds)
        rownames(margdata) <- c('EV_B','SD_B')
