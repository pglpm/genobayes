...
%% Last-Updated: 2018-11-13T13:54:02+0100
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Report-no: ***
\newif\ifarxiv
\arxivfalse
...
mechanisms and our uncertainty about them.

Let's focus for the moment the simplest reduction of such a graph, with
only one particular \snp\ with two alleles $\ya$, $\yb$, and one symptom
$S$. In an arbitrarily large population, consider the fraction $f_{S\|\ya}$
of individuals that show symptom $S$ among those having allele $\ya$, and
the fraction $f_{S\|\yb}$ among those having allele $\yb$. These are
...
These conditional frequencies are far easier to study than causal
relations, given the conditional frequencies in a population sample.


In this study we show how to quantify our \dobs\ about such conditional
frequencies in an arbitrarily large population, given
\begin{enumerate*}[label=(\arabic*)]
\item the conditional frequencies in a population sample, \item a
  representation of our initial information about such frequencies.
\end{enumerate*}
The idea behind the calculation is simple: using Bayes's theorem we have
\begin{multline}
...
frequencies in an arbitrarily large population, it is easy to see whether we
...
This approach is not dichotomous, unlike a \enquote{significance} test.
Rather, we will find a graduation of cases: from frequencies predicted to
be clearly distinct, to frequencies with uncertainties too large for
drawing definite conclusions. These cases can be sorted, obtaining a
sequence of \snp s with a decreasing belief of causal association with the
symptom. How many of these \snp s are to be selected for further study
depends on one's experimental and computational resources.
...
\mynote{Add some remarks about the role of these \enquote{limit
    frequencies}. Relation to partial exchangeability and belief about next
  individual in an endless sequence.}

Consider for definiteness symptomn $\ysA$ and a particular \snp\ with
alleles $\ya$, $\yb$. The limit conditional frequencies are denoted
$f_{\ysA\|\ya}$ and $f_{\ysA\|\yb}$. Denote the sample data by $\yD$ and
our initial beliefs by $\yI$. We want to quantify our joint belief about
these frequencies, expressed by the density function
\begin{equation}
...
individuals with allele $\ya$ shows symptom $\ysA$, and a fraction
$1-f_{\ysA\|\ya}$ doesn't show that symptom. Then, upon sampling $N_{\ya}$
individuals with allele $\ya$, our belief that a fraction $n_{\ysA\|\ya}$
of these will show symptom $\ysA$ and a fraction $1-n_{\ysA\|\ya}$ won't
show this symptom is
\begin{equation}
  \label{eq:sampling_belief}
  \pf(n_{\ysA\|\ya} \| N_{\ya}, f_{\ysA\|\ya}, \yI)
  =
  {f_{\ysA\|\ya}}^{N_{\ya}\,n_{\ysA\|\ya}} \;
  {(1-f_{\ysA\|\ya})}^{N_{\ya}\,(1-n_{\ysA\|\ya})},
\end{equation}
and analogously for allele $\yb$. Our belief about obtaining data
$\yD = (n_{\ysA\|\ya}, n_{\ysA\|\yb})$ is therefore
\begin{equation}
  \label{eq:sampling_belief_combined}
  \pf(\yD \| N_{\ya}, N_{\yb}, f_{\ysA\|\ya},f_{\ysA\|\yb}, \yI)
  =
 \prod_{x=\ya,\yb} {f_{\ysA\|x}}^{N_{x}\,n_{\ysA\|x}} \;
  {(1-f_{\ysA\|x})}^{N_{x}\,(1-n_{\ysA\|x})}.
\end{equation}

\medskip

Our initial \dob\ about the limit conditional frequencies is based on these
assumptions:
\begin{enumerate}[label=(\arabic*)]
\item we expect the frequencies conditional on the two alleles not to be
  wildly different in comparison to the full range $\clcl{0,1}$;
\item besides the point above, we don't want to make any special
  assumptions about the values of the two conditional frequencies.
\end{enumerate}
The first assumption is conservative; therefore, if our updated belief
...
The kind of density representing this initial belief
is qualitatively shown in \fig~\ref{fig:initial_belief}.
\begin{figure}[t!]%{r}{0.4\linewidth} % with wrapfigure
 \centering\includegraphics[width=0.75\linewidth]{prior3d.png}
\caption{Initial belief}\label{fig:initial_belief}
\end{figure}% scripts2/plots_distributions.nb
Mathematically we write it as an integral:
\begin{equation}
  ...
  \dA(\yA,\yq),
\end{equation}
where $\dbeta(\dotv \| \yA,\yq)$ is a beta density with shape
...
\begin{equation}
  ...
  f^{\yA\yq}\;(1-f)^{\yA\,(1-\yq)},
\end{equation}
and $\dA$ is a density that we leave unspecified for the moment: several
different expressions for $\dA$ will be used, to test how much our
inferences depend on our initial belief.

We write $\pf(f_{\ysA\|\ya}, f_{\ysA\|\yb} \| \yI)$ in the above fashion,
with integrals, only to emphasize that our beliefs about $f_{\ysA\|\ya}$ and
$f_{\ysA\|\yb}$ are not disjoint, and to show how their mutual dependence
comes about: by first considering two independent densities having the same
parameters, and then mixing over these parameters.

This construction has the interesting consequence that the our updated
belief $\pf(f_{\ysA\|\ya}, f_{\ysA\|\yb} \| \yD, \yI)$ can be formally
written in the following way:
\begin{equation}
  \label{eq:formal_hierarchic}
  \pf(f_{\ysA\|\ya}, f_{\ysA\|\yb} \| \yD, \yI) =
  \tint\di\yA\tint\di\yq\;
  \pf(f_{\ysA\|\ya}, f_{\ysA\|\yb} \| \yA,\yq,\yD, \yI)\;
  \pf(\yA,\yq \|\yD, \yI),
\end{equation}
with
\begin{equation}
  \label{eq:update_hyperparameter}
  \pf(\yA,\yq \|\yD, \yI) \propto
  \tint\di f_{\ysA\|\ya}\tint\di f_{\ysA\|\yb}\;
  \pf(\yD \|f_{\ysA\|\ya}, f_{\ysA\|\yb}, \yI)\;
  \pf(f_{\ysA\|\ya}, f_{\ysA\|\yb} \|\yA,\yq, \yI).
\end{equation}
This representation is discussed in appendix***. It is as if $\yA$, $\yq$
were unknown parameters, and our update for the frequencies proceeded by
first updating our belief about the parameters and then marginalizing them
out. This is a so-called \emph{hierarchic} model \citep{good1980}. A point
rarely emphasized in the literature is that there is no mathematical
difference between a hierarchic and a non-hierarchic model: we could forget
about the integrals in formula~\eqref{eq:beta_gamma_prior} and about the
update formula~\eqref{eq:formal_hierarchic}, and simply treat
$\pf(f_{\ysA\|\ya}, f_{\ysA\|\yb} \| \yI)$ as the density depicted in
\fig~\ref{fig:initial_belief}, with update~\eqref{eq:intuitive_Bayes}. The
results would be the same. The hierarchic way of thinking, though, often
helps in constructing densities that better represent our initial beliefs,
and also leads to formulae that can be better approximated when exact
computation is unfeasible.
