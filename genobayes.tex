...
%% Last-Updated: 2018-09-11T21:46:12+0200
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Report-no: ***
\newif\ifarxiv
\arxivfalse
...
to $5.3 \times 10^6$ or $740 \times 10^6$. Our initial information $\yI$ is
that each individual is characterized by an insomnia parameter
...
$C\defd 8\times 2^{94} \approx 1.6 \times 10^{29}$ possible values.
...
\end{equation}
From these we compute the mutual information \citep[there called
\enquote{rate of transmission}]{shannon1948}[\chap~2]{coveretal1991_r2006}:
\begin{equation}
...
completely irrelevant to one another, and is equal to the entropy of either
marginal probability distribution if they are deterministically related.
Thus, instead of asking whether they are \enquote{uncorrelated} or
\enquote{correlated}, we're quantifying their mutual relevance.
...
the joint relative-frequency distribution of the variates in the full
population. Denote the relative frequency for $\ys$ in the full population
by $\yFs$, that for $\yg$ by $\yFg$, and their joint frequency distribution
by $\yF$. Denote the corresponding relative-frequency distributions in the
sampled population by $\yfs$, $\yfg$, and $\yf$. Note that $\ysum\yF=1$ and
so on for the other frequency distributions. The joint
probability~\eqref{eq:joint_prob_new_ind} \emph{when $\yF$ is known} is
given as a special case of the multivariate hypergeometric distribution:
\begin{equation}
  \label{eq:joint_prob_new_ind_hyperg}
  \pf\bigl(\ysi{0}, \ygi{0} \| \yf, n,\yF,N, \yI\bigr) \equiv
  \pf\bigl(\ysi{0}, \ygi{0} \| \yF,N, \yI\bigr) 
  =X_{\ysi{0}, \ygi{0}}.
\end{equation}
...
\begin{multline}
  ...
  \pf\bigl(\ysi{0}, \ygi{0} \| \yf, n,N, \yI\bigr)  =
  \sum_{\yF}  \pf\bigl(\ysi{0}, \ygi{0} \| \yf, n,\yF,N, \yI\bigr) \;
  \pf( \yF \| \yf, n,N, \yI) \equiv{}\\
  \sum_{\yF}  X_{\ysi{0}, \ygi{0}}\,  \pf( \yF \| \yf, n,N, \yI).
\end{multline}
...
$\pf\bigl(\ysi{0} \| \yf, n,N, \yI\bigr)$ by replacing $\yF$, $\yf$ with
$\yFs$, $\yfs$; and for $\pf\bigl(\ygi{0} \| \yf, n,N, \yI\bigr)$ by
...
We need to assess our initial probability $\yI$ about $\yF$,
$\pf(\yF \| N,\yI)$. As a first tentative let's assume that this
probability is constant for all possible distributions $\yF$. There are
$\binom{N+C-1}{C-1}$ possible distributions $\yF$
\citep[\sect~2.1]{csiszaretal2004b}; hence
\begin{equation}
  \label{eq:constant_prior}
  \pf(\yF \| N,\yI) = \binom{N+C-1}{C-1}^{-1}.
\end{equation}
...
\begin{equation}
  ...
  \pf(\yF \| \yf, n, N,\yI) = \binom{N+C-1}{n+C-1}^{-1}\,\yprod\binom{N\yF}{n\yf}.
\end{equation}

Using the last probability distribution and the multivariate
hypergeometric~\eqref{eq:multivariate_hyperg} with $n=1$,
$(x_i)=(\delt_{\yv,i})$, we can calculate the probability that a newly
...
\begin{multline}
  ...
  ={}\\
  \sum_{\yF} \frac{N X_{\yv}-n x_{\yv}}{N-n}\, \binom{N+C-1}{n+C-1}^{-1}\,\yprod\binom{N\yF}{n\yf}.
\end{multline}
