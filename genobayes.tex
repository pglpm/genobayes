...
%% Last-Updated: 2018-09-11T13:44:18+0200
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Report-no: ***
\newif\ifarxiv
\arxivfalse
...
considering their joint probability for a new individual $n+1$, given our data:
\begin{equation}
  \label{eq:joint_prob_new_ind}
  \pf\bigl(\ysi{n+1}, \ygi{n+1} \| \yD, \yI\bigr),
\end{equation}
and the marginal probabilities
\begin{equation}
  \label{eq:marginals_new_ind}
  \pf\bigl( \ygi{n+1} \| \yD, \yI\bigr),
  \qquad
    \pf\bigl(\ysi{n+1} \| \yD, \yI\bigr).
\end{equation}
...
\begin{multline}
  ...
  \mI\bigl( \ysi{n+1} : \ygi{n+1} \| \yD,\yI \bigr) \defd{}\\
  \sum_{\mathclap{\ysi{n+1},\ygi{n+1}}} \pf\bigl(\ysi{n+1}, \ygi{n+1} \| \yD, \yI\bigr)
 \; \ln\frac{\pf(\ysi{n+1}, \ygi{n+1} \| \yD, \yI)}{
    \pf(\ysi{n+1} \| \yD, \yI)\;
    \pf(\ygi{n+1} \| \yD, \yI)}.
\end{multline}
...
population is $\yF$, where $\yF$ has $C$ components and $\ysum\yF=1$; that
is, $N F_{\yv}$ individuals have value $\yv$ for this variate. There are
$\binom{N+C-1}{C-1}$ possible distributions
...
\begin{multline}
  ...
  ={}\\
  \sum_{\yF} \frac{NF_{\yv}-nf_{\yv}}{N-n}\, \binom{N+C-1}{n+C-1}^{-1}\,\yprod\binom{N\yF}{n\yf}.
\end{multline}
