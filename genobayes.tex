...
%% Last-Updated: 2018-09-23T00:16:24+0200
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Report-no: ***
\newif\ifarxiv
\arxivfalse
...
\end{equation}
This yields a constant  initial probability distribution for $(\yso,\ygo)$:
\begin{multline}
...
frequency $x_{\yso,\ygo}$ in the data and the initial probability $1/C$,
...
$\yI_{\yCg'} \ne \yI_{\yCg''}$, which yield different initial probabilities
for the marginal frequency distribution of a single gene. For example, for
very large $N$, if we consider a single gene, $\yCg=2$, the probability
density for its marginal frequency $f$ is constant in $\di f$. If we
consider an additional gene, $\yCg=4$, the probability density for the
marginal frequency of the previous gene becomes $6 f\, (1-f)\,\di f$. If we
consider an additional third gene, this probability density will be again
...
marginal probability for the insomnia symptoms alone given the full set of
data, \eqn~\eqref{eq:marginals_final_const_prior}, depends on the number of
gene variations considered in the data. Likewise, the probability for the
...
doesn't lead to different marginal probability distributions for the
...
is characterized by a Dirichlet probability distribution for $\yF$ in the
$N \to \infty$ limit:
\begin{equation}
...
\end{equation}
This distribution has the property of leaving the probability distribution
...
doesn't assign equal probabilities to the possible frequency distributions
...
\end{multline}
Note, however, that the probability density for the frequency of controls
vs cases is uniform.
