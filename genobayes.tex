...
%% Last-Updated: 2018-09-14T11:59:20+0200
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Report-no: ***
\newif\ifarxiv
\arxivfalse
...
\begin{equation}
  ...
  \pf\bigl(\ysi{0}, \ygi{0} \| \yf, n,N, \yIc\bigr)  =
  \frac{
    \sum_{\yF} X_{\ysi{0}, \ygi{0}}\,
...
\begin{equation}
  ...
  \sum_{\yF} X_{\ysi{0}, \ygi{0}}\, \yprod\binom{N\yF}{n\yf}
  =
  \frac{n x_{\ysi{0}, \ygi{0}}+1}{N} \binom{N+C}{n+C}
...
\begin{equation}
  ...
  \pf\bigl(\ysi{0}, \ygi{0} \| \yf, n,N, \yIc\bigr)  =
\frac{(N+C) n x_{\ysi{0}, \ygi{0}}+N-n}{N\,(n+C)}
\end{equation}

For large $C$ this is approximately
\begin{equation}
    \label{eq:joint_prob_new_ind_full_const_prior_final_approx_C}
    \pf\bigl(\ysi{0}, \ygi{0} \| \yf, n,N, \yIc\bigr)  \approx
    \frac{n x_{\ysi{0}, \ygi{0}}}{N} +
    \frac{(N-n)\,(n x_{\ysi{0}, \ygi{0}} +1)}{N C}
\end{equation}
and for large $N$
\begin{equation}
    \label{eq:joint_prob_new_ind_full_const_prior_final_approx_N}
    \pf\bigl(\ysi{0}, \ygi{0} \| \yf, n,N, \yIc\bigr)  \approx
    \frac{n x_{\ysi{0}, \ygi{0}} +1}{n + C}.
\end{equation}

For the marginal distributions we obtain, by summation,
\begin{gather}
  \label{eq:marginals_final_const_prior}
    \pf\bigl(\ysi{0} \| \yf, n,N, \yIc\bigr)  =
\frac{(N+C) n s_{\ysi{0}}+(N-n)\yCg}{N\,(n+C)},
\\
    \pf\bigl(\ygi{0} \| \yf, n,N, \yIc\bigr)  =
\frac{(N+C) n g_{\ygi{0}}+(N-n)\yCs}{N\,(n+C)}.
\end{gather}
Note that if the initial probability distribution for $(\ysi{0}, \ygi{0})$
is constant, then the corresponding marginals for $\ysi{0}$ and $\ygi{0}$
...
\begin{multline}
  ...
  \mI\bigl( \ysi{0} : \ygi{0} \| \yD,\yIc \bigr) =
  \ln[N\,(n+C)] +
  \sum_{\mathclap{\ysi{0},\ygi{0}}}
  \frac{(N+C) n x_{\ysi{0}, \ygi{0}}+N-n}{N\,(n+C)}\times{}\\
  \ln\frac{(N+C) n x_{\ysi{0}, \ygi{0}}+N-n}{
    [(N+C) n s_{\ysi{0}}+(N-n)\yCg]\,
    [(N+C) n g_{\ygi{0}}+(N-n)\yCs]
  }
\end{multline}
