...
%% Last-Updated: 2018-09-10T15:03:40+0200
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Report-no: ***
\newif\ifarxiv
\arxivfalse
...
\section{Approach via the joint probability}
\label{sec:joint_prob}

\paragraph{Notation}
The following notation produces compact but readily understandable
formulae. Functions and operations on tuples $\yx \defd (x_1,\dotsc,x_D)$,
$\yy \defd (y_1,\dotsc,y_D)$, and numbers $a$ operate component-wise. For
example:
\begin{subequations}
    \label{eq:example_notation}
  \begin{equation}
    \begin{gathered}
      \exp\yx \defd \bigl( \exp x_1, \dotsc, \exp x_D \bigr)
      \qquad
      \yx\yy \defd ( x_1\,y_1, \dotsc, x_D\,y_D )
      \\
      a\yx \defd (a x_1, \dotsc, a x_D)
      \qquad
      \yx^a \defd \bigl( {x_1}^a,\dotsc,{x_D}^a \bigr)
      \qquad\text{and so on.}
    \end{gathered}
  \end{equation}
The exception are the sum and multiplication operators $\ysum$, $\yprod$:
\begin{equation}
  \ysum\yx \defd x_1+\dotsb+x_D
  \qquad
  \yprod\yx \defd x_1  \dotsm x_D
\end{equation}
so that, for example, \[\ysum\ln(\yx/\yy) \defd \sum_{i=1}^D \ln(x_i/y_i).\]
Note also the conventions
  \begin{equation}
    \binom{a}{\yx} \defd \binom{a}{x_1, \dotsc, x_D} \defd
    \frac{a!}{x_1! \dotsm x_D!}
    \qquad
    \yprod\binom{\yy}{\yx} \defd \binom{y_1}{x_1}  \dotsm  \binom{y_D}{x_D},
  \end{equation}
\end{subequations}
...
We approach our problem first using finite exchangeability: we are sampling from
the Norwegian or European population. 

Denote by $N$ the amount of Norwegian or European population, roughly equal
to $5.3 \times 10^6$ or $740 \times 10^6$. Our initial information is that
each individual is characterized by an insomnia parameter
$\ys \in \set{1,\dotsc,8}$ and $\yng\defd 94$ gene allele pairs, whose
combinations are denoted
$\yg \defd (g_1,\dotsc,g_{\yng})\in \set{0,1}^{\yng}$. The combined variate
$\yv \defd (\ys,\yg)$ can thus assume
$D\defd 8\times 2^{94} \approx 1.6 \times 10^{29}$ possible values.

Suppose that the relative distribution of the variate $\yv$ in the whole
population is $\yF$, where $\yF$ has $D$ components and $\ysum\yF=1$; that
is, $N F_{\yv}$ individuals have value $\yv$ for this variate. There are
$\binom{N+D-1}{D-1}$ possible distributions
\citep[\sect~2.1]{csiszaretal2004b}. Most components of $\yF$ vanish
because there are vastly more variate values than individuals. The
probability that a sample of $n$ individuals has a relative distribution
$\yf$ of the variate is given by the multivariate hypergeometric
distribution\mynote{add refs}:
\begin{equation}
...
We want to infer the distribution $\yF$ given the distribution $\yf$ of a sample
of size $n\defd 6029$. Using Bayes's theorem,
\begin{equation}
...
\end{equation}
We need to assess our initial probability $\yI$ about $\yF$, $\pf(\yF \|
N,\yI)$. As a first tentative let's assume that this probability is
constant for all possible distributions $\yF$:
\begin{equation}
  \label{eq:constant_prior}
  \pf(\yF \| N,\yI) = \binom{N+D-1}{D-1}^{-1}.
\end{equation}
Substituting this and~\eqref{eq:multivariate_hyperg} into Bayes's
theorem~\eqref{eq:F_from_f_generic} and simplifying we obtain\mynote{add
  refs for summation formula}
\begin{equation}
  \label{eq:F_from_f_specific}
  \pf(\yF \| \yf, n, N,\yI) = \binom{N+D-1}{n+D-1}^{-1}\,\yprod\binom{N\yF}{n\yf}.
\end{equation}

Using the last probability distribution and the multivariate
hypergeometric~\eqref{eq:multivariate_hyperg} with $n=1$,
$(f_i)=(\delt_{\yv,i})$, we can calculate the probability that a newly
sampled individual presents a variate value $\yv$:
\begin{multline}
  \label{eq:prob_individual}
  \pf(\yv \| \yf,n,N,\yI) =
  \sum_{\yF} \pf(\yv \| \yF-\yf,N-n,\yI)\,\pf(\yF \| \yf, n, N,\yI)
  ={}\\
  \sum_{\yF} \frac{NF_{\yv}-nf_{\yv}}{N-n}\, \binom{N+D-1}{n+D-1}^{-1}\,\yprod\binom{N\yF}{n\yf}.
\end{multline}
...
a prior probability distribution for the frequency distributions that is
