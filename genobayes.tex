...
%% Last-Updated: 2018-09-18T22:40:54+0200
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Report-no: ***
\newif\ifarxiv
\arxivfalse
...
We can divide this sum into three: one sum over the pairs $(\ysi{0},
\ygi{0})$ which appear in our data, for which $x_{\ysi{0}, \ygi{0}}>0$; one
sum over the remaining $\ysi{0}$; one over the remaining $\ygi{0}$:
\begin{multline}
  ...
  \begin{aligned}[b]
  &+
    \!\begin{multlined}[t][0.95\linewidth]
\sum_{\mathclap{(\ysi{0},\ygi{0}) \in \yD}}
  \frac{(N+C) n x_{\ysi{0}, \ygi{0}}+N-n}{N\,(n+C)}\times{}\\
  \ln\frac{(N+C) n x_{\ysi{0}, \ygi{0}}+N-n}{
    [(N+C) n s_{\ysi{0}}+(N-n)\yCg]\,
    [(N+C) n g_{\ygi{0}}+(N-n)\yCs]
...
&+\widebar{C}\,
  ...
  \ln(N-n)
  \\
  &-\sum_{\mathclap{\ysi{0} \text{ remaining}}}
  \frac{N-n}{N\,(n+C)}\,
  \ln [(N+C) n s_{\ysi{0}}+(N-n)\yCg]
  \\
  &-\sum_{\mathclap{\ygi{0} \text{ remaining}}}
  \frac{N-n}{N\,(n+C)}\,
  \ln [(N+C) n s_{\ygi{0}}+(N-n)\yCs],
  \end{aligned}
\end{multline}
where $\widebar{C}$ is the number of pairs \emph{not} present in the data,
that is, those for which $x_{\ysi{0}, \ygi{0}}=0$.
