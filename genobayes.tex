...
%% Last-Updated: 2018-09-23T17:08:35+0200
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Report-no: ***
\newif\ifarxiv
\arxivfalse
...
\begin{equation}
  ...
  X_{\yso, \ygo},
\end{equation}
which is a special case of a multivariate hypergeometric distribution --
\enquote{drawing from an urn without replacement}\mynote{add refs}. The
conditional in the \dob\ indicates that we know $\yF$ and $N$ besides
our initial knowledge $\yI$, and the first equality says that the data
$\yD$ would be irrelevant if we knew $\yF$.

If $\yF$ is unknown, then by the theorem of total \dob\ we have
\begin{multline}
...
can therefore appear in the data, that is, have frequencies $g_{\yg}>0$ and
consequently $x_{\ys,\yg}>0$. Denote by $\yCgd$ the number of distinct
...
our \dobs\ about them separately. The mutual information vanishes if the
symptoms and genetic variations are completely irrelevant to one another,
and is equal to the least of the entropies
$\sH\bigl(\yso \| \yD,\yI\bigr)$, $\sH\bigl(\ygo \| \yD,\yI\bigr)$ if one
variate gives us complete certainty about the other.
...
obviously consider initial states of knowledge such that
\begin{equation}
...
Our first step is to calculate the joint
...
population. The joint \dob\ \emph{when $\yF$ is known} is given as a
special case of the multivariate hypergeometric distribution
\eqref{eq:joint_prob_new_ind_F_known}. If the frequency distribution $\yF$
isn't known, from the theorem of total \dob\ we obtain
\eqn~\eqref{eq:joint_prob_new_ind_F_unknown}. We must therefore find our \dob\ in $\yF$ given the data. It can be found using Bayes's
theorem:
\begin{equation}
...
Bayes's theorem requires two \dobs: the one for the
frequency distribution $\yf$ of the sampled population, given $\yF$; and
our initial one for $\yF$. The first is given by the multivariate
...
\begin{multline}
  ...
  \pf\bigl(\yso, \ygo \| N,\yIc\bigr)  =
  \sum_{\yF}  \pf\bigl(\yso, \ygo \| \yF,N, \yIc\bigr) \;
  \pf( \yF \| N, \yIc) ={}\\
...
\begin{equation}
  ...
  \pf\bigl(\yso, \ygo \| \yf, n,N, \yIc\bigr)  \propto
...
\iffalse For large $C$ this is approximately
\begin{equation}
...
\end{gather}
Note that if the initial  distribution of \dob\ for $(\yso, \ygo)$
...
denoted with \enquote{$\yCg$} because it depends on the number of gene
variations we consider. Two different values $\yCg'$ and $\yCg''$
correspond to two different states of knowledge,
$\yI_{\yCg'} \ne \yI_{\yCg''}$, which yield different initial \dobs\
for the marginal frequency distribution of a single gene. For example, for
very large $N$, if we consider a single gene, $\yCg=2$, the 
density of \dob\ for its marginal frequency $f$ is constant in $\di f$. If we
consider an additional gene, $\yCg=4$, the  density of \dob\ for the
marginal frequency of the previous gene becomes $6 f\, (1-f)\,\di f$. If we
consider an additional third gene, this  density will be again
different. As a consequence, if we decide to consider additional genes we cannot
compare our inference with the ones previously made, because they come from
contradictory initial states of knowledge.
...
It would be advantageous to consider an initial state of knowledge that
doesn't lead to different marginal  distributions of \dobs\ in the
frequencies when we want to consider an additional gene variation, and that
doesn't present the counter-intuitive features above.

One such a state of knowledge $\yIo$ exists (perhaps it isn't unique) and
is characterized by a Dirichlet  distribution for $\yF$ in the
$N \to \infty$ limit:
\begin{equation}
  \label{eq:dirichlet_prior}
  \pf(\yF \| \yIo)\,\di\yF = \frac{1}{\yprod\Gamma\bigl( \frac{2}{C} \bigr)}
  \yprod \yF^{\frac{2}{C}-1}\,\delt(\ysum\yF-1)\, \di\yF
\end{equation}
This distribution has the property of leaving the  distribution of \dob\
for the marginal frequencies of any number of gene variations invariant if
we consider one more gene, thanks to the marginalization properties of the
Dirichlet distribution \citep[\sect~2 p.~211]{ferguson1973}. It moreover
assigns a uniform distribution to the frequency of each gene variation. It
doesn't assign equal \dobs\ to the possible frequency distributions
for the $\yCs\equiv 8$ symptom combinations, but rather
\begin{multline}
  \label{eq:dirichlet_prior_symptoms}
  \pf(\yFs \| \yIo)\,\di\yFs = \frac{1}{\yprod\Gamma\bigl( \frac{2}{\yCs} \bigr)}
  \yprod \yFs^{\frac{2}{\yCs}-1}\,\delt(\ysum\yFs-1)\,\di\yFs ={}\\
  \frac{1}{8\Gamma\bigl( \frac{1}{4} \bigr)}
  \yprod \yFs^{-3/4}\,\delt(\ysum\yFs-1)\,\di\yFs.
\end{multline}
Note, however, that the  density of \dob\ for the frequency of controls
vs cases is uniform.
...
$(C-1)$-dimensional simplex; but the density~\eqref{eq:dirichlet_prior} is
