...
spread <- function(q){sapply(1:numsymptomvariants,function(co){max(abs(sapply(1:(numsnpvariants-1),function(x){sapply((x+1):numsnpvariants,function(y){(q[co,x]-q[co,y])/(q[co+numsymptomvariants,x]+q[co+numsymptomvariants,y])})})))})}
