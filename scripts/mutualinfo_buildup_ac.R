...
## parameter A approx chosen via Bayes's theorem
...
for(ngenes in totalgenes){
    ...
    write.table(cbind(genegroup,allminfo),paste0(savedir,'iteration_afromc_',ngenes,'.csv'),sep=',',row.names=F,col.names=F,na='Infinity')
    ...
    write.table(cbind(keptgenes,keptinfo),paste0(savedir,'infoseq_afromc_',ngenes,'.csv'),sep=',',row.names=F,col.names=F,na='Infinity')
}
stopCluster(cl)
