...
for(i in allsymptoms){
    print(i)
    meanf <- mean(data[,i])
    spread <- matrix(NA,nrow=ncombinations,ncol=7)
    colnames(spread) <- c('gene1','gene2','max','min','spread','rms_mean','rms_auto')
    ...
    for(g1 in allgenes[-length(allgenes)]) {
        for(g2 in allgenes[-c(1:g1)]) {
            ...
            fmax <- max(values)
            fmin <- min(values)
            drms <- sqrt(mean((values-meanf)^2))
            arms <- sqrt(mean(unlist(sapply(1:3,function(x){sapply((x+1):4,function(y){(values[x]-values[y])^2})}))))
            spread[ii,] <- c(g1,g2,fmax,fmin,fmax-fmin,drms,arms)
        }}

write.csv(spread,paste0('diffmeasures_2genes_s',c('A','B','C')[i],'.csv'))
