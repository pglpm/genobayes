...
for(ngenes in totalgenes){
    ...
    write.table(cbind(genegroup,allminfo),paste0(savedir,'iteration_',aa,'_',ngenes,'.csv'),sep=',',row.names=F,col.names=F,na='Infinity')
    ...
    write.table(cbind(keptgenes,keptinfo),paste0(savedir,'infoseq_',aa,'_',ngenes,'.csv'),sep=',',row.names=F,col.names=F,na='Infinity')
}
stopCluster(cl)
