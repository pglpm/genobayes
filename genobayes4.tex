...
%% Last-Updated: 2019-08-26T16:53:32+0200
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\newif\ifarxiv
\arxivfalse
...
relative link strengths between some insomnia symptoms and a specific set of \snp
...
the link strength between variates. Section~\ref{sec:methods} summarizes
...
\begin{figure}[t!]%{r}{0.4\linewidth} % with wrapfigure
 \centering\includegraphics[width=0.49\linewidth]{cons_post_diag_list.png}%
 \includegraphics[width=0.49\linewidth]{unif_post_diag_list.png}
...
Note that in both cases most probability mass is actually above the
diagonal: the spreads of the distributions appears larger than they
actually are, owing to the high number of samples and the physical print
size of the points.
...
frequencies and its 10-quantile, which is our
measure~\eqref{eq:significance_measure}. Such quantiles are shown, sorted,
in \fig~\ref{fig:relevance_sorted} for all \snp-symptom pairs. For each
pair we show the result for the two pre-data states of knowledge, joined by
a line.
...
\caption{Sampling plots of our two initial states of knowledge, 100\,000
  ...
  conservative belief~\eqref{eq:conservative_prior}.}\label{fig:not-initial_beliefs}
\end{figure}% scripts2/plot_priors.nb
...
case is
\begin{equation}
  \label{eq:not-final_from_uniform}
...
\begin{subequations}
  \label{eq:not-final_from_conservative}
  \begin{gather}
  ...
  \end{multlined}\\
  \shortintertext{with}
      \label{eq:not-posterior_parameters}
  \begin{multlined}[t][\linewidth]
...
\begin{subequations}
  \label{eq:not-stats_final_unif}
  \begin{align}
    \label{eq:not-mean_final_unif}
    &\E(f_{|x}\| \yD, \yIu) =
    \frac{N_{x}\,F_{|x}+1}{N_{x}+2},
    \frac{N_{\yb}\,F_{|\yb}+1}{N_{\yb}+2}
    \qquad x=\ya,\yb,
    \\
    \label{eq:not-var_final_unif}
    &\var(f_{|x} \| \yD, \yIu) =
    \frac{(N_{x}F_{|x}+1)\,[N_{x}\,(1-F_{|x})+1)}{(N_{x}+2)^2\,(N_{x}+3)}
    \qquad x=\ya,\yb,
    \\
    \label{eq:not-cov_final_unif}
...
notation
\begin{equation}
  \label{eq:not-notation_avg_parameters}
  \uav{X(\yua,\yub)} \defd
      \int_{\mathrlap{0}}^{\mathrlap{\infty}}\di\yua
    \int_{\mathrlap{0}}^{\mathrlap{\infty}}\di\yub\,
...
\begin{subequations}
  \label{eq:not-stats_final_conserv}
  \begin{align}
    \label{eq:not-mean_final_conserv}
    &\E(f_{|x} \| \yD, \yIc) =
                                 \uav{\frac{N_{x}\,F_{|x}+\yua}{N_{x}+\yua+\yub}}
                                 \qquad x=\ya,\yb,
\\
  \label{eq:not-var_final_conserv}
...
\end{multlined}
    \\
    \label{eq:not-cov_final_conserv}
...
\caption{\enquote{Relevance} of the 94 \snp s for the three symptoms, sorted.}\label{fig:relevance_sorted}
\end{figure}% scripts2/plot_results.nb
